const { cardsPriorityMap1, cardTypes } = require("./constants");
const fs = require("fs");
...
const sortCardsByPriority = (a, b) => {
  const priorityA = cardsPriorityMap1[a];
  const priorityB = cardsPriorityMap1[b];
...
const sortCardsByLetter = (a, b) => {
  ...
  for (let i = 0; i < 5; i++) {
    const priorityA = cardsPriorityMap1[cardA[i]];
    const priorityB = cardsPriorityMap1[cardB[i]];
